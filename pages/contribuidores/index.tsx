...
interface IContributorsPage {
  contributors
}

const Image: React.FC<ImageProps> = ({ src, indice }) => {
  const key = '191e83'
  const thumbnail = `
...
const ContributorsPage: React.FC<IContributorsPage> = ({ contributors }) => {
  return (
      ...
      >
        ...
        >
          {contributors.map(({ user, projectUrl }, indice) => (
            <Widget style={{ maxWidth: '400px' }} key={user}>
